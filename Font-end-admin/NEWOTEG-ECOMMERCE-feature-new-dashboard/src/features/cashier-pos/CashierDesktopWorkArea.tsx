...
function TicketStep({ flow }: { flow: CashierCheckoutFlow }) {
  ...
  const seller = [ticket.vendeur?.prenom, ticket.vendeur?.nom].filter(Boolean).join(' ') || ticket.vendeurId || 'Vendeur';
  return (
  ...
  return (
    <section className="min-h-0 overflow-y-auto px-8 py-6">
      ...
      <div className="relative mt-6 flex">
        ...
        <input inputMode="tel" value={flow.customerQuery} onChange={event => flow.setCustomerQuery(event.target.value)} placeholder="+237 6 99 12 34 56" className="h-16 min-w-0 flex-1 rounded-l-md border border-slate-300 bg-white pl-12 pr-4 text-2xl font-medium outline-none focus:border-primary" />
        <button type="button" className="h-16 rounded-r-md border border-l-0 border-primary px-8 font-bold text-primary">Rechercher</button>
...
function SummaryPanel({ flow }: { flow: CashierCheckoutFlow }) {
  ...
  return <aside className="min-h-0 overflow-y-auto border-l border-slate-200 bg-white px-6 py-7"><h2 className="text-xl font-bold">Finaliser</h2><div className="mt-8 space-y-6 text-slate-700"><p className="flex gap-3"><ReceiptText size={21} />Ticket {ticket.numeroTicket}</p><p className="flex gap-3"><UserRound size={21} />{clientName}</p><p className="flex gap-3"><FileText size={21} />{documentLabel(flow.documentType)}</p><p className="flex gap-3"><Banknote size={21} />{paymentLabel(flow.method)}</p></div><div className="mt-8 flex items-center justify-between border-t border-slate-200 pt-7"><span className="text-lg">Total</span><strong className="text-2xl">{money(flow.total)}</strong></div><button type="button" disabled={!flow.canPay} onClick={() => void flow.checkout()} className="mt-8 flex min-h-16 w-full items-center justify-center gap-3 rounded-md bg-primary px-5 text-lg font-bold text-white disabled:opacity-40">{flow.submitting ? <Loader2 className="animate-spin" /> : <Printer />}Valider et imprimer</button><button type="button" onClick={() => flow.setStep('TICKET')} className="mt-5 w-full text-sm font-medium text-primary">Mettre en attente</button></aside>;
}

function SuccessStep({ flow, onNextTicket, onPrint }: { flow: CashierCheckoutFlow; onNextTicket: () => void; onPrint: () => void }) {
  ...
  return <>
    <section className="min-h-0 overflow-y-auto px-8 py-6 text-center"><CheckCircle2 className="mx-auto text-emerald-600" size={48} /><h1 className="mt-3 text-4xl font-bold">Paiement validé</h1><p className="mt-3 text-5xl font-bold tracking-wide text-primary">{money(flow.total)}</p><div className="mx-auto mt-6 max-w-xl border border-slate-300 text-left"><dl className="grid grid-cols-2 divide-x divide-y divide-slate-200 text-sm"><dt className="p-3 text-slate-600">Ticket</dt><dd className="p-3 font-medium">{flow.selected?.numeroTicket}</dd><dt className="p-3 text-slate-600">Client</dt><dd className="p-3 font-medium">{clientName}</dd><dt className="p-3 text-slate-600">Document</dt><dd className="p-3 font-medium">{documentLabel(flow.documentType)} {document?.numero}</dd><dt className="p-3 text-slate-600">Méthode de paiement</dt><dd className="p-3 font-medium">{paymentLabel(flow.method)}</dd>{flow.method === 'ESPECES' && <><dt className="p-3 text-slate-600">Monnaie rendue</dt><dd className="p-3 font-medium">{money(flow.change)}</dd></>}</dl></div><p className="mx-auto mt-4 max-w-xl bg-emerald-50 py-3 text-sm font-medium text-emerald-700"><Check size={18} className="mr-2 inline" />Vente enregistrée · stock mis à jour</p><button type="button" onClick={onNextTicket} className="mx-auto mt-4 min-h-14 w-full max-w-xl rounded-md bg-primary text-lg font-bold text-white">Traiter le ticket suivant</button><div className="mx-auto mt-3 grid max-w-xl grid-cols-3 gap-3"><button type="button" onClick={onPrint} className="flex h-12 items-center justify-center gap-2 rounded-md border border-slate-300"><Printer size={18} />Imprimer</button><button type="button" onClick={share} className="flex h-12 items-center justify-center gap-2 rounded-md border border-slate-300"><MessageCircle size={18} />WhatsApp</button><button type="button" onClick={onPrint} className="flex h-12 items-center justify-center gap-2 rounded-md border border-slate-300"><FileDown size={18} />PDF</button></div></section>
    <aside className="min-h-0 overflow-y-auto border-l border-slate-200 bg-white px-7 py-6"><h2 className="text-xl font-bold">Document prêt</h2><div className="mt-5 border border-slate-300 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><img src="/logo.png" alt="Newoteg" className="h-16 w-16 object-contain" /><strong className="text-xl">NEWOTEG</strong></div><div className="mt-5 flex justify-between"><strong>{documentLabel(flow.documentType).toUpperCase()}</strong><span>{document?.numero || flow.selected?.numeroTicket}</span></div><p className="mt-6 text-sm">Client</p><p className="mt-1 font-medium">{clientName}</p><div className="mt-6 divide-y divide-slate-200 border-y border-slate-300">{flow.selected?.lignes.map(line => <div key={line.id} className="grid grid-cols-[1fr_auto] gap-3 py-3 text-xs"><span>{line.nomProduit} × {line.quantite}</span><span>{money(line.sousTotal)}</span></div>)}</div><div className="mt-4 flex justify-between text-xl"><strong>TOTAL</strong><strong>{money(flow.total)}</strong></div></div><p className="mt-5 flex items-center gap-3 text-sm text-slate-700"><CheckCircle2 className="text-emerald-600" />ORIGINAL · document généré</p><p className="mt-4 flex items-center gap-3 text-sm text-slate-700"><Printer />Imprimante prête</p></aside>
  </>;
}
...
export function CashierDesktopWorkArea({ flow, onNextTicket }: { flow: CashierCheckoutFlow; onNextTicket: () => void }) {
  ...
  const document = flow.result?.facture;
  return <>
    {flow.step === 'SUCCESS' ? <SuccessStep flow={flow} onNextTicket={onNextTicket} onPrint={() => setPrintOpen(true)} /> : <>{flow.step === 'TICKET' && <TicketStep flow={flow} />}{flow.step === 'CUSTOMER' && <CustomerStep flow={flow} />}{flow.step === 'PAYMENT' && <PaymentStep flow={flow} />}<SummaryPanel flow={flow} /></>}
    {printOpen && document && <ReceiptGenerator documentId={document.id} printCount={document.printCount || 0} type={flow.documentType === 'FACTURE' ? 'facture' : flow.documentType === 'BON_VENTE' ? 'bonVente' : 'ticket'} numero={document.numero} dateVente={document.dateEmission} methodePaiement={flow.method} montantTotal={flow.total} client={document.client ? { nom: `${document.client.nom} ${document.client.prenom || ''}`.trim(), telephone: document.client.telephone } : undefined} lignes={(document.lignes || []).map((line: any) => ({ nomProduit: line.nomProduit, quantite: line.quantite, prixUnitaire: Number(line.prixUnitaireTTC), sousTotal: Number(line.sousTotalTTC) }))} onClose={() => setPrintOpen(false)} />}
  </>;
}
