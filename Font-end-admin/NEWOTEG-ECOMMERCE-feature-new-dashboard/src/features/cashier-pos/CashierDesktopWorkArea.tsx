...
function SummaryPanel({ flow, onCheckoutAndPrint }: { flow: CashierCheckoutFlow; onCheckoutAndPrint: () => Promise<void> }) {
  ...
  return <aside className="min-h-0 overflow-y-auto border-l border-slate-200 bg-white px-6 py-7"><h2 className="text-xl font-bold">Finaliser</h2><div className="mt-8 space-y-6 text-slate-700"><p className="flex gap-3"><ReceiptText size={21} />Ticket {ticket.numeroTicket}</p><p className="flex gap-3"><UserRound size={21} />{clientName}</p><p className="flex gap-3"><FileText size={21} />{documentLabel(flow.documentType)}</p><p className="flex gap-3"><Banknote size={21} />{paymentLabel(flow.method)}</p></div><div className="mt-8 flex items-center justify-between border-t border-slate-200 pt-7"><span className="text-lg">Total</span><strong className="text-2xl">{money(flow.total)}</strong></div><button type="button" disabled={!flow.canPay} onClick={() => void onCheckoutAndPrint()} className="mt-8 flex min-h-16 w-full items-center justify-center gap-3 rounded-md bg-primary px-5 text-lg font-bold text-white disabled:opacity-40">{flow.submitting ? <Loader2 className="animate-spin" /> : <Printer />}Valider et imprimer</button>{flow.error && <p role="alert" className="mt-4 flex items-start gap-2 bg-red-50 p-3 text-sm font-medium text-red-700"><AlertCircle className="mt-0.5 shrink-0" size={18} />{flow.error}</p>}<button type="button" onClick={() => flow.setStep('TICKET')} className="mt-5 w-full text-sm font-medium text-primary">Mettre en attente</button></aside>;
}
