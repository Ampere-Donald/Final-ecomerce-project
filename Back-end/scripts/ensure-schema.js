...
const SQL_STATEMENTS = [
  ...
  // Recherche produit rapide pour la caisse et la vente
  `CREATE EXTENSION IF NOT EXISTS pg_trgm;`,
  `CREATE INDEX IF NOT EXISTS "idx_produit_nom_trgm" ON "produit" USING GIN ("nom_produit" gin_trgm_ops);`,
  `CREATE INDEX IF NOT EXISTS "idx_produit_designation_en_trgm" ON "produit" USING GIN ("designation_en" gin_trgm_ops);`,
  `CREATE INDEX IF NOT EXISTS "idx_produit_marque_trgm" ON "produit" USING GIN ("marque" gin_trgm_ops);`,
  `CREATE INDEX IF NOT EXISTS "idx_produit_code_trgm" ON "produit" USING GIN ("code" gin_trgm_ops);`,
  `CREATE INDEX IF NOT EXISTS "idx_produit_code_famille_trgm" ON "produit" USING GIN ("code_famille" gin_trgm_ops);`,
  ...
  `WITH daily_tickets AS (
    SELECT SUBSTRING("numero_ticket" FROM '^T-([0-9]{8})-') AS "period",
  ...
  // ── Inventaire par catégorie/famille + délégation ──
  `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'StatutInventaire') THEN CREATE TYPE "StatutInventaire" AS ENUM ('EN_COURS', 'VALIDE', 'ANNULE'); END IF; END $$;`,
  ...
  `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ligne_inventaire_inventaire_id_fkey') THEN ALTER TABLE "ligne_inventaire" ADD CONSTRAINT "ligne_inventaire_inventaire_id_fkey" FOREIGN KEY ("inventaire_id") REFERENCES "inventaire"("id") ON DELETE CASCADE ON UPDATE CASCADE; END IF; END $$;`,
  ...
  // ── Bon de commande fournisseur + désignation anglaise ──
  `ALTER TABLE "produit" ADD COLUMN IF NOT EXISTS "designation_en" VARCHAR(200);`,
  `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'StatutCommandeFournisseur') THEN CREATE TYPE "StatutCommandeFournisseur" AS ENUM ('BROUILLON', 'ENVOYEE', 'RECUE', 'ANNULEE'); END IF; END $$;`,
  ...
  `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ligne_commande_fournisseur_commande_id_fkey') THEN ALTER TABLE "ligne_commande_fournisseur" ADD CONSTRAINT "ligne_commande_fournisseur_commande_id_fkey" FOREIGN KEY ("commande_id") REFERENCES "commande_fournisseur"("id") ON DELETE CASCADE ON UPDATE CASCADE; END IF; END $$;`,
